...
import { createFolder, createVideoRecord, getFolderContents, getFolderTree, getVideoById, ensureOwnsFolder, listPendingVideos } from './services/drive.js';
...
app.get('/api/folders/:folderId/contents', requireUser, async (request, response) => {
    ...
    const folderId = requestedFolderId && requestedFolderId !== 'root' ? requestedFolderId : null;
    try {
...
app.post('/api/videos/upload-session', requireUser, async (request, response) => {
    const { folderId, title, fileName, mimeType, sizeBytes } = request.body;
...
app.post('/api/videos/upload', requireUser, upload.single('file'), async (request, response) => {
    ...
    const folderId = rootFolderId(request.body.folderId);
    if (!file) {
...
async function syncVideoFromBunny(videoId) {
...
async function processBunnyUpload({ videoId, bunnyVideoId, buffer }) {
    try {
